...
export default function NutritionPage() {
    ...
    useEffect(() => {
        authFetch("http://localhost:8000/api/profile/")
            .then((res) => res.json())
            .then((data) => {
    ...
    const fetchMealPlan = async () => {
        setLoading(true);
        try {
            const res = await authFetch("http://localhost:8000/api/ai-meal-plan/", {
                method: "POST",
    ...
    const handleSubstituteDish = async (mealKey, dishIndex, dishName) => {
        try {
            const res = await authFetch("http://localhost:8000/api/ai-substitute-dish/", {
                method: "POST",
