...
export default function WorkoutSchedule() {
    ...
    const fetchLibrary = async () => {
        try {
            const res = await authFetch("http://localhost:8000/api/exercise-library/");
    ...
    const fetchExercises = async (dateStr) => {
        try {
            ...
            const res = await authFetch(`http://localhost:8000/api/workouts/?date=${dateStr}`);
                            ...
                            <div className="bg-white shadow rounded mt-1 w-full text-sm border z-10">
                                <button
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            const res = await authFetch("http://localhost:8000/api/ai-generate-workout/", {
                                                method: "POST",
                                                headers: {
                                ...
                                <button
                                    onClick={async () => {
                                        ...
                                        try {
                                            setIsGenerating(true);
                                            const res = await authFetch("http://localhost:8000/api/workouts/reset/", {
                                                method: "POST",
                                                headers: {
                                                    "Content-Type": "application/json",
                                                },
                                            ...
                                            {list.map((ex, i) => (
                                                <div key={i} className="border p-2 rounded flex justify-between items-center">
                                                    ...
                                                    <button
                                                        onClick={async () => {
                                                            try {
                                                                const res = await authFetch(`http://localhost:8000/api/workouts/${workoutId}/add-exercise/`, {
                                                                    method: "POST",
                    ...
                    >
                        {deleteMode && (
                            <button
                                onClick={async () => {
                                    try {
                                        const res = await authFetch(`http://localhost:8000/api/workouts/${workoutId}/exercise/${ex.id}/`, {
                                            method: "DELETE",
                                        });
